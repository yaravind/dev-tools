...
if [[ -e "$HOMEBREW_PREFIX" && ! -d "$HOMEBREW_PREFIX" ]]; then
  error "${HOMEBREW_PREFIX} exists but is not a directory."
  exit 1
fi
...
if [[ "$PREPARE_PREFIX_OWNERSHIP" -eq 1 ]]; then
  ...
  if [[ ! -d "$HOMEBREW_PREFIX" ]]; then
    error "${HOMEBREW_PREFIX} was not created successfully."
    exit 1
  fi
...
if [[ ! -x "$BREW_BIN" ]]; then
  error "Unable to find brew binary under ${HOMEBREW_PREFIX}/bin or PATH."
  exit 1
fi
...
if [[ "$PREPARE_PREFIX_OWNERSHIP" -eq 1 ]]; then
  ...
  if [[ "$(stat -f '%Su' "$HOMEBREW_PREFIX")" != "$USER" ]]; then
    error "${HOMEBREW_PREFIX} is not owned by $USER."
    exit 1
  fi
