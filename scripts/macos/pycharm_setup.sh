...
print_structured_report() {
  ...
  printf "${THEME_HEADER}──────────────────────────────────────────────────────────────────────────────${RESET}\n"
  printf '  %-24s %s\n' "Script" "PyCharm Plugin Setup (macOS)"
  printf '  %-24s %s\n' "Mode" "--${MODE}"
  printf '  %-24s %s\n' "Config" "$PYCHARM_PLUGIN_FILE"
  printf '  %-24s %s\n' "Launcher" "${PYCHARM_CLI[1]}"
  ...
  if (( ${#dependency_order[@]} == 0 )); then
    ...
    printf "${THEME_HEADER}Suggested dependency entries to add to config/pycharm.txt${RESET}\n"
...
print_structured_report "$overall_status" "$attempted_total"
