...
# Read pinned items directly from the TaskBar shortcuts directory (most reliable and comprehensive)
...
if (Test-Path $taskbarDir) {
    ...
    foreach ($shortcut in (Get-ChildItem -Path $taskbarDir -Filter "*.lnk" -ErrorAction SilentlyContinue)) {
        try {
            ...
            # Handle AUMID-based shortcuts (Microsoft Store apps e.g. Outlook, Netflix)
            if ($arguments -match 'shell:AppsFolder\\(.+)') {
                $pinnedItems += "AUMID:$($Matches[1])"
            } elseif ($target) {
                $pinnedItems += $target
            } else {
    ...
    Write-Info "Falling back to Shell.Application to enumerate pinned apps..."
    # Fallback: use Shell.Application (may not enumerate all apps)
    $shell = New-Object -ComObject Shell.Application
    $taskbarFolder = $shell.Namespace(0x1F3F)
    if ($taskbarFolder) {
        foreach ($item in $taskbarFolder.Items()) {
            if ($item.Path) { $pinnedItems += $item.Path }
        }
    } else {
        Write-Warn "Could not access Taskbar pinned folder via Shell.Application."
    }
}
